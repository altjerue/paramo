subroutine turBlaz(params_file,output_file,cool_withKN,with_abs)
   ...
   real(dp) :: uB,uext,R,gmin,gmax,numin,numax,pind,B,D,g1,g2,tstep,Qnth,tmax,&
         ...
         L_jet,volume,sigma,beta_bulk,L_B,eps_B,f_rec,urad_const,f_esc,eps_acc
   real(dp),allocatable,dimension(:) :: freqs,t,Ntot,Inu,gg,dt,nu_obs,t_obs,&
         dg,urad
   real(dp),allocatable,dimension(:,:) :: dotg,nn,jnut,jmbs,jssc,jeic,ambs,&
         anut,Qinj,Ddiff
   logical :: with_cool
   ...
   allocate(t(0:numdt),freqs(numdf),Ntot(numdt),Inu(numdf),dt(numdt),&
         nu_obs(numdf),t_obs(numdt),dg(numbins),urad(numbins))
...
call h5io_wdble0(group_id,'Rdis',Rdis,herror)
