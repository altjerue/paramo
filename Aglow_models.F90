...
subroutine afterglow_SPN98(nuo, to, z, E0, epse, epsB, G0, pind, n, d_lum, adiab, flux)
!
!  Description:
...
evolution: do i=1, size(t)
   ...
   spec: do j = 1, size(nu)
      ...
      cool_fast_or_slow: if ( tdy < t0 ) then
         if ( nu_c > nu(j) ) then
      ...
      end if cool_fast_or_slow
   end do spec
end do evolution
end subroutine afterglow_SPN98
!##############################################################################
