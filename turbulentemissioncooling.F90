program turbulentemission
   ...
   real(dp) :: p,tstep,tmax,numin,numax,gmin,gmax,sig,gam0,Gamma0,Gamma2,&
         Gammah,Gammaa,va,Rva,kk,tc,R,B_0,th,thss,n0,Bulk_lorentz,l_rho,rho,&
         nuext,Uph,Uph_co,tcm,t2,tcorg,mfp,B_rms
   real(dp),allocatable,dimension(:) :: t,dt,Ap,Dpp,Diff,gdotty,g,dg,total,nuj,&
         dnuj,tempg,tempnu,Rarray,Mgam,zero1,zero2,U,Inu
   ...
   write(*,*) "tmax: ",tmax,"sig: ",sig,"tc: ",tc,"R: ",R,"Theta: ",Th,"va: ",va,"sigmaT",sigmaT
   ...
   time_loop: do i=1,numt
      ...
      !    write(*,*) "COOLING"
      !    Diff=2d0*((Gamma0*gam0**2+Gamma2*g**2)/tc)*( (tc*1.5d0/t(i))**p )
      !    gdotty=-(Ap+2d0*g*((tc*1.5d0/t(i))**p)/tc+(Diff/g)-g**2/(gam0*tc))
      ...
      !$OMP& PRIVATE(j)
      do j=1,numf
         call mbs_emissivity(jmbs(i,j),nuj(j),g,n1(i,:),B_0)
         !!ssc needs to be in a seperate loop
         !call mbs_absorption(ambs(i,j),nuj(j),g,n1(i,:),B_0)
      end do
      !$OMP END PARALLEL DO
      write(*,*) "test3"
